...
object MinHash {
  ...
  def randomHashFunction(seed: Int, _M: Int = 32): HashFunc[Int] = {
    val rand = new scala.util.Random(seed)
    new HashFunc[Int] {
      private[this] val M = _M
      private[this] val a = (rand.nextLong() & ((1L << 62)-1)) * 2 + 1       // random odd positive integer (a < 2^w)
      private[this] val b = math.abs(rand.nextLong() & ((1L << (64 - M))-1)) // random non-negative integer (b < 2^(w-M)
