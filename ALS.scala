...
object ALS {
  ...
  def apply(R: Array[Array[Int]], factors: Int, α: Float, λ: Float, iterations: Int) =
    new ALS_Unary(new UnaryMatrix(R), factors, α, λ, iterations).run

  /** Sparse matrix whose values are all ones. It's column major just like CSCMatrix */
  protected class UnaryMatrix(val data: Array[Array[Int]]) { self =>
  ...
  protected final class ALS_Unary(val R: UnaryMatrix, val factors: Int, val α: Float, val λ: Float, val iterations: Int) extends ALS[UnaryMatrix] {
  ...
  protected final class ALS_Full(val R: CSCMatrix[Float], val factors: Int, val α: Float, val λ: Float, val iterations: Int) extends ALS[CSCMatrix[Float]] {
  ...
  protected abstract class ALS[Dataset] {
